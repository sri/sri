I build software, explore ideas, and ship useful things. 
Some of these apps are my family/kid. And some of them get used everyday.

## Recent projects

- [sri/orgline](https://github.com/sri/orgline) - a simple outliner, backend a single go binary writing to SQLite; demo: https://sri.github.io/orgline/index.html 
- [sri/timecops](https://github.com/sri/timecops) - multiple timers on a single page, config thru url params so you can bookmark and share it; demo: https://sri.github.io/timecops/timecops.html
- hoopboard - an online board to teach/learn plays - https://sri.github.io/hoopboard/
- A fun way to decide something: https://help-me-decide.pages.dev/
- For my kid, when he has a time-consuming, difficult homework; a fun way to do [Pomodoro](https://en.wikipedia.org/wiki/Pomodoro_Technique): https://pomofun.com
- For my kid, to play basketball as different teams: https://sri.github.io/scoreboard/index.html
- List of all the AI personalities: https://whotheai.com/
- Glist - how I do shopping: https://sri.github.io/glist/#shopping
- [sri/yt_highlights](https://github.com/sri/yt_highlights) - how I gather hightlights of games on YouTube: https://github.com/sri/yt_highlights
